...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: isZh ? '象棋开局库：10 种主流布局详解' : 'Xiangqi Openings: 10 Main Opening Systems Explained',
    description: isZh
      ? '当头炮、屏风马、顺炮、列手炮、仙人指路……逐一详解每种象棋开局的标准记谱、战略思路与常见应手。'
      : 'Central Cannon, Screen Horses, Identical Cannons, Adjacent Soldier and more — each Xiangqi opening explained with notation, strategy, and common replies.',
    alternates: localeAlternates('xiangqi/openings', locale),
    openGraph: {
      title: isZh ? '象棋开局库' : 'Xiangqi Openings',
      description: isZh ? '10 种主流象棋开局详解。' : '10 main Xiangqi opening systems explained.',
    ...
    <div className="yb-container" style={{ paddingBlock: 'var(--space-12)' }}>
      ...
      <header style={{ maxWidth: '64ch' }}>
        ...
        <p className="yb-lead" style={{ marginTop: 'var(--space-3)' }}>
          {isZh
            ? '开局决定中局走向。下面汇总 10 种主流象棋开局，每个都有独立详解页，包含标准记谱、战略思路与常见应手。'
            : 'The opening shapes the middlegame. Here are 10 main Xiangqi opening systems — each has its own page with notation, strategic ideas, and common replies.'}
        </p>
      </header>
