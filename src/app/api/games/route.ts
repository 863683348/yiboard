 ...
 * 用引擎重放收到的棋谱，终局胜负/和棋必须与上报 result 一致，否则拒收。
 */
export async function POST(request: Request) {
  ...
  // 解析棋谱：格式如 "H8,I9,G7"，任一非法落点即拒
  const points = movesRaw
    ? movesRaw.split(',').map((n) => fromNotation(n))
    : [];
  ...
  const finalState = replay(points as NonNullable<(typeof points)[number]>[]);
